...
//lanjutan setelah function logout
	...
	function update_buku(){
		...
		if($this->form_validation->run() != false){
			...
			$data['buku']= $this->db->query("select * from buku B, kategori K where B.id_kategori=K.id_kategori and B.id_buku='$id'")->result();
			$data['kategori'] = $this->M_perpus->get_data('kategori')->result();
			echo "validasi tidak valid";exit();
			$this->load->view('admin/header');
			$this->load->view('admin/editbuku',$data);
			$this->load->view('admin/footer');
		}
	...
	function tambah_anggota_act(){
		...
		if ($this->form_validation->run() != false){
			...
			$data['anggota'] = $this->M_perpus->get_data('anggota')->result();
			redirect(base_url().'admin/anggota', $data);
		}
	}
	...
	function update_anggota(){
		...
		$this->form_validation->set_rules('nama_anggota','Nama Anggota','required|min_length[4]');
		$this->form_validation->set_rules('alamat','Alamat','required|min_length[4]');
		$this->form_validation->set_rules('no_telp','No_Telp','required|min_length[4]');
		if ($this->form_validation->run() != false){
			$where = array('id_anggota' => $id);
			$data = array(
					'nama_anggota' => $nama_anggota,
	...
	function transaksi_selesai($id){
		$data['buku']= $this->M_perpus->get_data('buku')->result();
		$data['anggota']= $this->M_perpus->get_data('anggota')->result();
		$data['peminjaman']=$this->db->query("SELECT * from transaksi t, anggota a, buku b
			WHERE a.id_anggota = t.id_anggota AND b.id_buku = t.id_buku AND t.id_pinjam = '$id'")->result();
		$this->load->view('admin/header');
	...
	function transaksi_selesai_act(){
		...
		if($this->form_validation->run()!=false){
			...
			$total_denda=$denda*$selisih;//update status peminjaman
			$data=array('status_peminjaman'=>'1','total_denda'=>$total_denda,'tgl_pengembalian'=>$tgl_dikembalikan,'status_pengembalian'=>'1');
			$w=array('id_pinjam'=>$id);

			$databuku = array('status_buku' => '1' );
			$wbuku = array('id_buku' =>'$buku');
			$this->M_perpus->update_data('transaksi',$data,$w);//update status buku
			$this->M_perpus->update_data('buku',$databuku,$wbuku);//update status buku
			$data2=array('status_buku'=>'1');
 	...
 	function laporan_transaksi(){
 		...
 		if($this->form_validation->run() !=false){
 			$data['laporan'] = $this->db->query("SELECT * from peminjaman p, detail_pinjam d, buku b, anggota a WHERE p.id_pinjam=d.id_pinjam and b.id_buku=d.id_buku and a.id_anggota=p.id_anggota 
 	...
 	function laporan_print_transaksi(){
 		$dari = $this->input->get_data('dari');
 		$sampai = $this->input->get_data('samapai');
 		if($dari != "" && $sampai != ""){
 	...
 	public function search_anggota(){
			$keyword_anggota = $this->input->post('keyword_anggota');
			$data['anggota'] = $this->M_perpus->get_keyword_anggota($keyword_anggota);
			$this->load->view('admin/header');
			$this->load->view('admin/anggota',$data);
			$this->load->view('admin/footer');
		}		
